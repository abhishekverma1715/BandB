...
const ProductCard = ({
  ...
  return (
    <div className="group relative bg-white rounded-2xl border border-[#E4E7EC] shadow-sm hover:shadow-md hover:border-[#174A8B]/40 hover:-translate-y-0.5 transition-all duration-300 flex flex-col overflow-hidden h-full">
      ...
      {/* Top Image Container with Full Edge-to-Edge Image */}
      <Link to={productUrl} className="relative h-60 w-full bg-[#F7F8FA] overflow-hidden block border-b border-[#E4E7EC]">
        ...
        {/* Top Left Badge */}
        {badge && (
          <span className={"absolute top-3.5 left-3.5 " + badgeColor + " text-white text-[10px] font-bold px-2.5 py-1 rounded-lg shadow-sm uppercase tracking-wider z-10"}>
            {badge}
          </span>
        )}
        ...
        <button
          ...
          className={"absolute top-3.5 right-3.5 w-8 h-8 rounded-full bg-white/90 backdrop-blur-sm border border-[#E4E7EC] shadow-sm flex items-center justify-center text-gray-400 hover:text-rose-500 hover:scale-105 transition-all duration-200 z-10 " + (isWishlisted ? 'text-rose-500 fill-rose-500' : '')}
        >
      ...
      <div className="p-5 flex flex-col flex-grow justify-between">
        <div>
          ...
          {/* Material Grade Subtitle */}
          <p className="text-xs text-[#667085] font-normal mb-4">
            {grade || 'Virgin PP / ABS'}
          </p>
        ...
        {/* Bottom Price & Get Bulk Quote Action Row */}
