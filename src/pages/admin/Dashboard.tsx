...
import { selectAllProducts, selectStockAlerts } from '../../features/products/productsSlice.js';
import {
  selectAllInquiries,
  selectUnreadCount,
...
const Dashboard: React.FC = () => {
  ...
  const recentInquiries = useAppSelector((state) => selectRecentInquiries(state, 5));
  ...
  const handleQuickStatusChange = (id: string | number, newStatus: InquiryStatus) => {
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
        {stats.map((stat, idx) => (
          <Link
            key={idx}
            to={stat.link}
            className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white border ${stat.borderColor} shadow-xs hover:shadow-md transition-all group relative overflow-hidden flex flex-col justify-between`}
          >
            <div>
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-body">
                  {stat.title}
                </span>
                <div className={`p-2 sm:p-3 rounded-xl sm:rounded-2xl ${stat.bgColor}`}>{stat.icon}</div>
              </div>
            ...
            <div className="mt-3 sm:mt-4 pt-2.5 sm:pt-3 border-t border-gray-100 flex items-center justify-between text-[11px] sm:text-xs text-body font-medium">
              <span className="truncate">{stat.subtext}</span>
              <FiArrowRight className="text-gray-400 group-hover:translate-x-1 group-hover:text-primary transition-all flex-shrink-0 ml-1" />
            </div>
          </Link>
        ))}
      </div>
            ...
            <tbody className="divide-y divide-[#E4E7EC]">
              {recentInquiries.length === 0 ? (
                ...
                recentInquiries.map((inq) => {
                  const itemKey = (inq._id || inq.id || inq.inquiryId) as string | number;
                  return (
                    <tr
                      key={String(itemKey)}
                    ...
                    >
                      ...
                      <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                        <select
                          value={inq.status}
                          onChange={(e) =>
                            handleQuickStatusChange(itemKey, e.target.value as InquiryStatus)
                          }
        ...
        {selectedInquiry && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs overflow-y-auto">
            <motion.div
              ...
              className="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-5 sm:p-8 shadow-2xl border border-gray-100 my-auto"
            >
              <div className="flex items-center justify-between pb-4 border-b border-gray-100">
                <div>
                  <span className="text-[10px] font-mono font-bold text-primary uppercase">
                    {selectedInquiry.inquiryId || selectedInquiry.id}
                  </span>
                  <h4 className="text-lg font-bold text-secondary">{selectedInquiry.subject}</h4>
                </div>
              ...
              <div className="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between">
                <Link to="/admin/messages" className="text-xs font-bold text-primary hover:underline">
