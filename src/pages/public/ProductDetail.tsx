...
const ProductDetail: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'specs' | 'logistics' | 'oem'>('specs');
  ...
  const product: Product | null = useMemo(() => {
    ...
    const found = products.find((p) => p.slug === slug || String(p.id) === slug);
    return found || products[0];
  }, [slug, products]);

  const basePriceNum = useMemo(() => {
    if (!product || !product.price) return 150;
    const match = product.price.match(/\d+(\.\d+)?/);
    return match ? parseFloat(match[0]) * 80 : 150;
  }, [product]);

  const moqNum = useMemo(() => {
    if (!product || !product.moq) return 100;
  ...
  const discountTier = useMemo(() => {
    ...
    if (quantity >= 1000) return { discount: 0.1, label: '10% Off (Volume Tier 2)' };
  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-[#F7F8FA] pt-32 pb-24 text-center">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl font-bold text-[#101828]">Loading Product...</h2>
        </div>
      </div>
    );
  }
  ...
  const handleWhatsAppInquiry = () => {
    ...
    window.open(`https://wa.me/919118913028?text=${encodeURIComponent(message)}`, '_blank');
  };
              ...
              <div className="bg-white rounded-2xl border border-[#E4E7EC] p-6 shadow-sm">
                ...
                <ul className="space-y-3 text-xs text-[#667085]">
                  <li className="flex items-start gap-2.5">
                    <span className="w-4 h-4 rounded-full bg-[#F0F4F8] text-[#174A8B] flex items-center justify-center font-bold text-[10px] flex-shrink-0 mt-0.5">
                      ✓
                    </span>
                    <span>
                      <strong>100% Virgin Resins:</strong> No degraded regrind content for maximum mechanical tensile strength.
                    </span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="w-4 h-4 rounded-full bg-[#F0F4F8] text-[#174A8B] flex items-center justify-center font-bold text-[10px] flex-shrink-0 mt-0.5">
                    ...
                    <span>
                      <strong>Custom Masterbatch Colors:</strong> Available for orders exceeding 1,000 units.
                    </span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="w-4 h-4 rounded-full bg-[#F0F4F8] text-[#174A8B] flex items-center justify-center font-bold text-[10px] flex-shrink-0 mt-0.5">
                    ...
                    <span>
                      <strong>In-Mold Labeling (IML):</strong> Custom corporate logo screen printing available.
                    </span>
                  </li>
                </ul>
              </div>
            ...
            <div className="lg:col-span-7 flex flex-col gap-6">
              <div className="bg-white rounded-2xl border border-[#E4E7EC] p-6 sm:p-8 shadow-sm">
                ...
                <p className="text-[#667085] text-sm sm:text-base leading-relaxed mb-6">
                  {product.description ||
                    'Precision engineered injection-molded container manufactured from premium grade virgin polymer granules, ensuring high structural load tolerance and chemical durability.'}
                </p>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-white/10">
                    <div>
                      ...
                      <div className="flex items-baseline gap-2 mt-1">
                        <span className="text-3xl sm:text-4xl font-black text-white">₹{unitPrice}</span>
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center text-xs mb-6">
                    <div
                      className={`p-2.5 rounded-xl border transition-all ${
                        quantity < 500
                          ? 'bg-[#174A8B] border-white/30 text-white font-bold'
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          value={quantity}
                          onChange={(e) =>
                            setQuantity(Math.max(moqNum, parseInt(e.target.value) || moqNum))
                          }
          ...
          <div className="mt-12 bg-white rounded-2xl border border-[#E4E7EC] p-6 sm:p-8 shadow-sm">
            <div className="flex gap-8 border-b border-[#E4E7EC] mb-8 overflow-x-auto">
              {[
                ...
                { id: 'logistics' as const, label: 'Packaging & Freight Logistics' },
                { id: 'oem' as const, label: 'Custom OEM & Printing Support' },
              ].map((tab) => (
                <button
                  key={tab.id}
            ...
            {activeTab === 'oem' && (
              <div className="p-6 rounded-xl bg-[#F7F8FA] border border-[#E4E7EC] text-sm leading-relaxed text-[#667085]">
                <h4 className="font-bold text-[#101828] text-base mb-2">
                  Custom OEM Mold &amp; Brand Printing Services
                </h4>
